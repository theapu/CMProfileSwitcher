...
public class TimedProfileFragment extends DialogFragment implements
	...
	private Button buttonResetTimedProfile;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		ProfileSwitcher.myLog("all profile Names - " + Arrays.toString(profileNames));
		...
		buttonResetTimedProfile = (Button) v.findViewById(R.id.buttonResetTimedProfile);
		...
		timePickerView.setHour(0);
		timePickerView.setMinute(30);
		buttonSetTimedProfile.setOnClickListener(this);
		buttonResetTimedProfile.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		case R.id.buttonSetTimedProfile:
			alarmTime.add(Calendar.HOUR_OF_DAY, timePickerView.getHour());
			alarmTime.add(Calendar.MINUTE, timePickerView.getMinute());
			break;
		case R.id.buttonResetTimedProfile:
		...
		if ( minutesToAdd > 0 ) {
			...
			duration.set(Calendar.HOUR_OF_DAY, timePickerView.getHour());
			duration.set(Calendar.MINUTE, timePickerView.getMinute());
			duration.add(Calendar.MINUTE, minutesToAdd);
			timePickerView.setHour(duration.get(Calendar.HOUR_OF_DAY));
			timePickerView.setMinute(duration.get(Calendar.MINUTE));
			return;
		}
